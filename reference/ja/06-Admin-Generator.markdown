...
慣習では、パーシャルにはプレフィックスとして(`_`)を、
コンポーネントにはプレフィックスとしてチルダ(``)をつけます:
...
プレフィックスが`executeBatch`であるキャメルケースの名前のメソッドを探します。
...
`action`を定義しない場合、adminジェネレーターは
プレフィックスが`executeList`であるキャメルケースの名前のメソッドを探します。
...
`action`を定義しない場合、adminジェネレーターは`executeListをプレフィックスとするキャメルケースの名前
のメソッドを探します。
...
`peer_count_method`オプションは現在のフィルター用のオブジェクトの個数を
...
`table_count_method`オプションは現在のフィルター用のオブジェクトの個数を算出するために
呼び出すメソッドを定義します。
