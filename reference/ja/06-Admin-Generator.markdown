...
`param`エントリは生成されたモジュール用の設定オプションを格納します。
...
`config`エントリを介してadminジェネレーターをカスタマイズするために使うことのできるすべての利用可能なオプションをこのドキュメントで説明します。
...
多くのオプションが引数としてフィールドのリストを受け取ります。
...
`NAME`の文字列は妥当なオブジェクトのゲッターメソッドの名前に使われる文字列になります(`get`の後にキャメルケースバージョンの`NAME`文字列が続く)。 
...
>他のゲッターに関して、オブジェクトを文字列に変換する意味のある`__toString()`メソッドを定義していればプレースホルダーとして使うことができます。

### 設定の継承

adminジェネレーターの設定はコンフィギュレーションカスケードの原則に基づきます。継承ルールは次の通りです:
...
`credential`オプション(下記を参照)を使用するユーザークレデンシャルに基づいて、(リストとフォーム上の)adminジェネレーターのアクションを隠すことができます。
...
この設定はすべてのページに対して定義されページごとにオーバーライドできます(`list`、`filter`、`form`、`edit`、と`new`)。
...
コールバックは`renderer_arguments`オプションで定義されたフィールドと引数の値で呼び出されます。
...
`display`オプションはlistで表示する順序付きカラムの配列を定義します。
...
`pager_class`オプションはlistを表示する際に使用するページャクラスを定義します。
...
`action`を定義しない場合、adminジェネレーターはプレフィックスが`executeBatch`であるキャメルケースの名前のメソッドを探します。
...
`action`を定義しない場合、adminジェネレーターはプレフィックスが`executeList`であるキャメルケースの名前のメソッドを探します。
...
`filter`セクションはlistページに表示されるフォームをフィルタリングするための設定を定義します。
...
>**TIP**
>`new`と`edit`セクションの両方で`class`オプションを定義できますが、1つのクラスを使用し条件ロジックを使用し違いを考慮する方がベターです。
