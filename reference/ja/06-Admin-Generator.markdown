...
たくさんのオプションがフィールドのリストを引数として受け取ります。フィールドは実際のカラム名もしくは
仮想的な名前になります。両方のケースにおいてゲッターは
モデルクラスで定義しなければなりません(`get`の後にキャメルケースのフィールド名が続く)。

コンテキストに基づいて、adminジェネレータはフィールドをレンダリングする方法を知っているほど十分に賢いです。
...
adminジェネレータはパーシャルとコンポーネントに対していくつかのパラメータを渡します:
  ...
  * `new`と`edit`ページに対して:
    ...
    * `form`:       現在のモデルオブジェクトに関連付けされているフォーム
...
プレースホルダーはパターン: `%%NAME%%`に従う文字列です。 
`NAME`の文字列は妥当なオブジェクトのゲッターメソッドに変換されるものになります。
(`get`の後にキャメルケースバージョンの`NAME`文字列が続く)。 
例えば、`%%title%%`は`$article->getTitle()`の値に置き換えられます。
...
しかしながら、リンクもしくはボタンが現れない場合、違法なアクセスからアクションは
適切にセキュアな状態でなければなりません。adminジェネレータの
クレデンシャルマネジメントは表示のみを考慮します。

`credential`オプションはlistページのカラムをかくすためにも使うことができます。
...
生成テンプレートはたくさんの`class`と`id`属性を定義するので
...
`display`オプションで表示されるカラムを指定する代わりに、 
こちらの方がカラムを隠すのに速いことがあります:
...
このオプションはフィールドをグループにアレンジするためにも使うことができます:
...
>**CAUTION**
>モデルフォームで定義されたすべてのフィールドは`display`オプションに存在しなければなりません。
