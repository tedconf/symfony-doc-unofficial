...
最初にマッチするルートが優先されるので、`routing.yml`設定ファイルで定義されるルートの順序は重要です。
...
クラスが提供されない場合、`sfRoute`基底クラスがフォールバックとして使われます。
...
`routing.yml`設定ファイルはルートを細かく設定するための複数のコンフィギュレーションをサポートします。
これらのコンフィギュレーションはそれぞれのルートをオブジェクトに変換するために`sfRoutingConfigHandler`クラスによって使われます。
...
`params`設定はルートに関連づけされるパラメーターの配列を定義します。
...
これはルートにマッチする処理の中でHTTPリクエストを強制します。
...
`type`オプションは必須でありモデルのために必要なルートの種類です。
...
`actions`オプションはルートに許可されるアクションの配列を定義します。
...
`prefix_path`オプションはすべての`url`パターンの先頭につけられるプレフィックスを指定します。
...
`show`アクションはルート用に許可されるアクションに含めなければならないか決定するために`with_show`オプションは`actions`オプションが`false`にセットされているときに使われます。
