...
`routing.yml`設定ファイルは名前つきのルートの定義の
リストを含みます:
...
`class`クラスは`sfRoute`基底クラスを継承します。提供されない場合、
...
ルートに使用するルートクラスを`class`設定によって変更できます。
...
([デフォルトでは`/`もしくは`.`](#chapter_05_sub_segment_separators))。
...
>この設定はコンフィギュレーションハンドラーによって自動的に`collection`にセットされます。
...
`method`オプションは必須です。このルートに関連するオブジェクトを読み取るために
...
モデルを適切なパラメーターの配列に変換するために呼び出すメソッドです。 
これは少なくともルートパターンの必須パラメーターを持つ配列を
...
`show`アクションはルート用に許可されたアクションに含めなければならないか決定するために
...
`with_wildcard_routes`オプションは2つのワイルドカードのルート:
(1つは単独のオブジェクト用、もう1つはオブジェクトコレクション用)を通して
アクションにアクセスできるようにします。
...
`collection_actions`オプションはコレクションルートで利用可能な
追加アクションの配列を定義します。
...
`object_actions`オプションはオブジェクトルートで利用可能な
追加アクションの配列を定義します。
