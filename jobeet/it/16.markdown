...
ora possono creare un account, ma ha bisogno di essere attivato dall'amministratore
prima di poter essere utilizzato. Pe far si che l'affiliato ottenga il suo token,
    ...
    class affiliateActions extends autoAffiliateActions
    {
      public function executeListActivate()
      {
        ...
        $message = $this->getMailer()->compose(
          ...
          <<<EOF
    Il tuo account affiliato di Jobeet è stato attivato.

    Il tuo token è {$affiliate->getToken()}.
...
Il metodo ~`compose()`~ prende quattro argomenti e ritorna un oggetto di messaggio
email:
...
>La classe `sfContext`
>
>L'oggetto ~`sfContext`~ contiene riferimenti a oggetti del core di symfony come
...
>proprie classi sarebbe meglio pensarci due volte, perchè introduce un ~forte accoppiamento|Forte Accoppiamento~. It is quite 
>È quasi sempre meglio passare l'oggetto che è necessario come argomento.
>
>È anche possibile utilizzare ~`sfContext`~ cmoe un registro e aggiungere i propri oggetti usando
>metodi `set()`. Prende un nome e un oggetto come argomenti e il metodo `get()`
...
impostazione predefinita, symfony registra un tester `mailer`  (~`sfMailerTester`~) cheto ease mail testing
facilita il test delle mail nei test funzionali.

Sostituire il file del test funzionale `affiliate` per l'applicazione di backend con
il seguente codice:
...
(`data/fixtures/030_affiliates.yml`) nella cartella `test/fixtures/`.

Ogni email inviata, può essere testata con l'aiuto dei metodi ~`checkHeader()`~ e
~`checkBody()`~. Il secondo argomento di `checkHeader()` e il primo
argomento di `checkBody()` può essere uno dei seguenti:

 * una stringa su cui verificare una corrispondenza esatta

 * una espressione regolare su cui verificare il valore

 * una espressione regolare negata (una espressione regolare che inizia con un `!`) per
...
> con il metodo ~`withMessage()`~. `withMessage()` prende un destinatario come primo argomento. Prende
