...
Visto che il libro si focalizzerà principalmente sul framework symfony, diamo per 
...
Visto che questo libro è stato scritto per symfony 1.3, andate
...
cartella `lib/vendor/` appena creata, ed estraetene i file:
    ...
    $ cd lib/vendor
    $ tar zxpf symfony-1.3-latest.tgz
...
![verifica configurazione](http://www.symfony-project.org/images/jobeet/1_3/01/configuration_check.png)
...
>Autoloading
>
>La classe `sfCoreAutoload` è un
>[~autoloader|Autoload~](http://www.php.net/autoload) PHP usato per caricare le classi
>core di symfony al volo. Per impostazione predefinita, symfony registra un altro autoloader
>per caricare automaticamente le classi salvate nella cartella `lib/`. Ciò significa che
>non si avrà mai bisogno di usare un `require` nel proprio codice. E' una delle numerose
>cose che symfony automatizza per gli sviluppatori.
...
![Un'eccezione nell'ambiente di sviluppo](http://www.symfony-project.org/images/jobeet/1_3/01/exception_dev.png)
...
Nel capitolo 21, imparerete come creare nuovi ambienti, come quello
di `stage`.
