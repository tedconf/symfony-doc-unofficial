...
Jobeet con l'~ORM~ ~Doctrine~ al posto di Propel. È semplice, poiché
...
delle ~risorse|Risorse~ (immagini, fogli di stile e JavaScript.). Quando si installa o si
abilita un nuovo plugin, occorre installare tali risorse tramite il
task `plugin:publish-assets`:

    $ php symfony plugin:publish-assets

Occorre anche cancellare la cartella `web/sf#PropelPlugin`:

    $ rm web/sf#PropelPlugin

>**TIP**
>Un'altra raccomandazione per l'uso di Doctrine al posto di Propel 
>è quella di cancellare i file `config/propel.ini` e `config/schema.yml`,
>in modo da a vere un'installazione pulita e senza riferimenti a Propel.
>
...
![Diagramma E-R](http://www.symfony-project.org/images/jobeet/1_2/03/diagram.png)
...
>**TIP**
>Si avrà bisogno di creare a mano la cartella `config/doctrine/` nel progetto,
...
Il file `config/~databases.yml~` contiene una connessione riferita a propel.
Siccome stiamo usando Doctrine, occorre cancellare `config/databases.yml`,
in modo da poterlo rigenerare per Doctrine.

    $ rm config/databases.yml

Ora eseguiamo i comandi seguenti per generare un nuovo file di configurazione
del database per Doctrine:
