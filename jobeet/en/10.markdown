...
Any website has ~forms|Forms~; from the simple contact form to the complex ones with
lots of fields. Writing forms is also one of the most complex and tedious task
for a web developer: you need to write the HTML form, implement validation
...
A symfony form is a class made of fields. Each field has a name, a ~validator|Validators~,
and a ~widget|Widgets~. A simple `ContactForm` can be defined with the following class:
...
>[widgets](http://www.symfony-project.org/api/1_3/widget) and
>[validators](http://www.symfony-project.org/api/1_3/validator). The API
...
The job form is a perfect example to learn ~form customization|Forms (Customization)~. 
...
For each field, symfony automatically generates a ~label|Form Labels~ (which will be used in
...
~template|Templates~ for the form is the same whether you want to create a new job or
...
We now have a form class and a template that renders it. Now, it's time to actually
make it work with some ~action|Action~s.
...
![Form flow](http://www.symfony-project.org/images/jobeet/1_3/10/form_flow.png)
...
When you browse to the `/job/new` page, a new ~form|Forms~ instance is created and
...
to the database (`$form->save()`), and the user is ~redirected|Redirection~ 
...
>**TIP**
>The `setTemplate()` method changes the ~template|Templates~ used for a given action. If
...
Everything must work fine by now. As of now, the user must enter the token
for the job. But the job token must be generated automatically when a new job
is created, as we don't want to rely on the user to provide a unique token.
...
The preview page is the same as the job page display. Thanks to the ~routing|Routing~,
...
![Not activated job](http://www.symfony-project.org/images/jobeet/1_3/10/not_activated.png)

![Activated job](http://www.symfony-project.org/images/jobeet/1_3/10/activated.png)
