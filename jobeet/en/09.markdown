...
Yesterday, we saw how to unit test our Jobeet classes using the lime
testing library packaged with symfony.
...
~test|Testing~ all the layers of an application: the routing, the model, the actions,
and the templates. They are very similar to what you probably already do
manually: each time you add or modify an action, you need go to the browser
...
something in your code, you must step through all the scenarios to ensure that you did
not break something. That's insane. Functional tests in symfony provide a way
to easily describe scenarios. Each scenario can then be played automatically over
and over again by simulating the experience a user has in a browser. Like unit
tests, they give you the confidence to code in peace.
...
implemented by the [~`sfBrowser`|Browser~](http://www.symfony-project.org/api/1_3/sfBrowser) 
class. It acts as a browser tailored for your application and directly connected to it,
...
methods like `getResponse()` and `getRequest()` but symfony provides a
better way.
...
class delegates the tests to **~tester|Testers~** objects. Several testers are bundled
with symfony, and you can also create your own.

As we saw yesterday, functional tests are stored under the
`test/functional/` directory. For Jobeet, tests are to be found in the
...
>**NOTE**
>We will describe more [testers](http://www.symfony-project.org/api/1_3/test)
...
![Tests on the command line](http://www.symfony-project.org/images/jobeet/1_3/09/cli_tests.png)
...
expired jobs from the homepage, we check that the ~CSS selector~
`.jobs td.position:contains("expired")` does not match anywhere in the
response HTML content (remember that in the fixture files, the only expired
job we have contains "expired" in the position). When the second argument of
the `checkElement()` method is a Boolean, the method tests the existence of
...
key can change between runs, we need to get the ##ORM## object from the database 
first.
...
created earlier. This class acts as a Domain Specific 
~functional tester class|Testers~ for Jobeet:
...
symfony provides the `~debug|Debug~()` method to output the response header and
content:
...
![Functional tests harness](http://www.symfony-project.org/images/jobeet/1_3/09/test_harness.png)
...
![Tests harness](http://www.symfony-project.org/images/jobeet/1_3/09/tests_harness.png)

See you Tomorrow
----------------

That wraps up our tour of the symfony test tools. You have no excuse
anymore to not test your applications! With the lime framework and the
functional test framework, symfony provides powerful tools to help you write
tests with little effort.

We have just scratched the surface of functional tests. From now on, each
time we implement a feature, we will also write tests to learn more
