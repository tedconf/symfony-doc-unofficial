...
There are two different kinds of automated ~test|Testing~s in symfony: 
**~unit tests|Unit Testing~** and **~functional tests|Functional Testing~**.
...
every single ~edge case|Edge Cases~? How can I be sure that everything is well tested?
But usually, the first question is much more basic: Where to start?
...
the ~code coverage|Code Coverage~ will rise, and you will become more confident 
about it. By starting with a pragmatic approach, you will feel more comfortable 
with tests over time. The next step is to write tests for new features. In no 
time, you will become test addict.
...
>**Note**: ~Windows~ command line unfortunately cannot highlight test results in
>red or green color.
...
We created the `~slug|Slug~ify()` method during day 5 to clean up a string so that it
...
failed; but if you have hundreds of tests in a file, it can be difficult
to quickly identify the behavior that fails.
...
The test description string is also an valuable tool when trying to figure
out what to test. You can see a pattern in the test strings: they are
sentences describing how the method must behave and they always start with the
method name to test.
...
`slugify()` method converts it to an empty string. So happy to have found the
cause, you open the `Jobeet` class and fix the problem right away. That's a
...
You cannot think about all ~edge cases|Edge Cases~ when writing tests, and that's fine.
But when you discover one, you need to write a test for it before fixing your
code. It also means that your code will get better over time, which is always
a good thing.
...
During day 1, we introduced the ~environment|Environments~s as a way to vary an application's
settings. By default, all symfony tests are run in the `test` environment, so
let's configure a different database for the `test` environment:
...
~task|Tasks~, but for tests, we need to reload the data each time we run them to put
...
The `test:unit` ~task|Tasks~ can also be used to launch all unit tests for a project:
