...
The [installation page](http://www.symfony-project.org/installation) on the
symfony website lists and compares all the available versions of symfony.

As this book has been written for symfony 1.3, go to the installation page for
...
![Configuration check](http://www.symfony-project.org/images/jobeet/1_3/01/configuration_check.png)
...
In symfony, **~applications|Application~** sharing the same data model are regrouped into
**projects**. For the Jobeet project, we will have two different applications:
a ~frontend|Frontend~ and a ~backend|Backend~.
...
The `generate:project` task generates the default ~structure|Structure~ of directories
...
>Autoloading
>
>The `sfCoreAutoload` class is a PHP
>[~autoloader|Autoload~](http://www.php.net/autoload) used to load symfony
>core classes on demand. By default, symfony also registers another autoloader
>to automatically load classes stored in the `lib/` directory. It means that
>you never need to use a `require` in your code. It is one of the numerous
>things that symfony automates for the developer.
...
controllers|Front Controller~**: all requests to the application are made
through them. But why do we have two front controllers as we have only defined
one application?

Both files point to the same application but for different environments. When
you develop an application, except if you develop directly on the production
server, you need several **environments**:
...
![An exception in the dev environment](http://www.symfony-project.org/images/jobeet/1_3/01/exception_dev.png)
...
framework comes bundled with three of them: `dev`, `test`, and `prod`. In
chapter 21, you will learn how to create new environments, such as the
`staging` one.
...
bad consequences of such a lazy attitude. If the user knows that your website
is developed with symfony, he will have access to a lot of sensitive files.
...
A good web practice is to put under the ~web root directory|Web Root
Directory~ only the files that need to be accessed by a web browser like
stylesheets, JavaScripts, and images. And by default, we recommend to store
these files under the `web/` sub-directory of a symfony project.

If you have a look at this directory, you will find some sub-directories for
web ~assets|Assets~ (css/ and images/) and the two front controller files. The
...
>**NOTE**
>The `/sf` ~alias|Alias (Apache)~ gives you access to images and javascript
...
This configuration makes Apache listen to port `8080` on your machine, so the
Jobeet website will be accessible at the following URL:
...
>~virtual hosts|Virtual Host~ instead of adding a new port each time you start
>a new project. Instead of choosing a port and add a `Listen` statement, choose
>a domain name and add a `ServerName` statement:
>
>     # This is the configuration for Jobeet
...
>     </VirtualHost>
>
>The domain name `jobeet.localhost` used in the Apache configuration has to be
>declared locally. If you run a Linux system, it has to be done in the
>`/etc/hosts` file. If you run Windows XP, this file is located in the
...
>**Note**
>If you have the Apache ~`mod_rewrite`|`mod_rewrite` (Apache)~
...
development environment and gives you access to a lot of information by
clicking on the different tabs: the current application configuration, the
logs for the current request, the SQL statements executed on the database
engine, memory information, and time information.
