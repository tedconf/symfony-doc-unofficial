...
As always, when you create a new resource, it's a good habit to define the
~URL~ first:
...
exception. This exception class is then automatically converted to a `~404|404
Error~` response. This is the simplest way to generate a `404` page from a
model class.
...
and development environment, two files are needed
...
rendered as a drop-down box thanks to the `sfWidgetFormChoice` widget. As seen
during day 10, we have changed the rendered tag by using the `expanded`
option.
...
method, it is time to ~refactor|Refactoring~ the code and create a
...
For the ~backend|Backend~, an `affiliate` module must be created for
affiliates to be activated by the administrator:

    $ php symfony propel:generate-admin backend JobeetAffiliate --module=affiliate

To access the newly created module, add a link in the main menu with the
number of affiliate that need to be activated:
...
Thanks to the ~REST~ architecture of symfony, it is quite easy to implement
web services for your projects. Although, we wrote code for a read-only web
service today, you have enough symfony knowledge to implement a read-write web
service.
...
token. Sending emails is the topic we will talk about tomorrow.
